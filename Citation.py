...
""""
print('Привет! Это будущий чат-бот с цитатами великих людей')
...
if alert != None:
    ...
    length_main = len(author_main) - 1
    if checkAuthor is None:
        print('Я не совсем понял запрос, возможно вы ищете что-то вроде этого:')
        print(quotes_main[random.randint(0, length_main)].text)
        print(author_main[random.randint(0, length_main)].text)
    else:
        author_site = requests.get('https://ru.citaty.net' + html_text.find('a', class_="w-10 mr-3 flex-none").get("href"))
        author_site_text = BeautifulSoup(author_site.text, 'html.parser')
        quotes = author_site_text.find_all('p', class_='blockquote-text')
        author = author_site_text.find_all('p', class_='blockquote-origin')
        length = len(author) - 1
        ...
        while button_next != None:
            ...
            author = author + author_site_text.find_all('p', class_='blockquote-origin')
            length = len(author) - 1
        print(quotes[random.randint(0, length)].text)
        print(author[random.randint(0, length)].text)


if alt_Name != None:
    print('Возможно, ' + alt_Name.text)
"""
